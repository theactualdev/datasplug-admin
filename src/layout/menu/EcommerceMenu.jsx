...
const MenuItem = ({
  ...
  // this removes all the "/" in the urls for sidebar links,
  // so as to maintain the active class for nested routes
  ...
  const matchedPath = useMemo(() => {
    ...
    if (link === "/") {
      ...
      newLink = link?.replace(/[^a-zA-Z0-9- ]/g, "");
    }
    return newLink;
  }, [link]);

  // this removes all the "/" in the current pathname,
  // so as to maintain the active class for nested routes
  const currentPath = useMemo(() => {
    ...
    if (pathname === "/") {
      ...
      newCurrentPath = pathname.replace(/[^a-zA-Z0-9- ]/g, "");
    }
  ...
  const menuItemClass = classNames({
    "nk-menu-item": true,
    "has-sub": sub,
    "active current-page": currentPath.includes(matchedPath),
    "nk-menu-hide": hideSideBarLink,
  });
  return (
  ...
  if (data === menu) {
    ...
    return (
      <React.Fragment>
        ...
        <li className={menuItemClass}>
          <Link to={`/`} className="nk-menu-link" onClick={() => setMenuData(menu)}>
            <span className="nk-menu-icon">
