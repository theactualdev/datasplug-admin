...
import { useGetStoreOrders } from "../../../../api/orders";
import {
  Button,
  Col,
  DataTable,
...
import { filterProductType, filterStatus } from "../order/OrderData";
import SortToolTip from "../tables/SortTooltip";

const StoreOrdersPage = ({ title = "All Orders" }) => {
  ...
  const { data: storeOrders, isLoading } = useGetStoreOrders(storeId, currentPage, itemsPerPage);
            ...
            <DataTableBody bodyclass="nk-tb-tnx">
              ...
              {storeOrders?.data?.map((item, idx) => (
                <DataTableItem key={idx}>
                  ...
                  <DataTableRow size="md">
                    <span className="tb-sub text-primary">{item.product?.productName}</span>
                  </DataTableRow>
                              ...
                              <li>
                                <DropdownItem
                                  ...
                                  onClick={(ev) => {
                                    ev.preventDefault();
                                    navigate(
                                      `${import.meta.env.PUBLIC_URL}/orders/order-details/${item._id}?type=single-order`
                                    );
                                  }}
                                >
                                  <Icon name="eye"></Icon>
            ...
            <PreviewAltCard>
              {storeOrders?.totalDocuments > 0 && (
                <PaginationComponent
                  ...
                  totalItems={storeOrders?.totalDocuments}
                  paginate={paginate}
